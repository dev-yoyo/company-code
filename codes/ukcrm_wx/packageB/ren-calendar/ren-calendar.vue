...
export default {
    ...
    watch: {
        date: function (newValue, oldValue) {
            this.y = newValue.substring(0,4)
            this.m = newValue.substring(5,7)
            let y = new Date().getFullYear() // 年
            let m = new Date().getMonth() + 1 // 月
            let d = new Date().getDate();
            if(y == this.y && m == this.m) {
              this.choose = `${y}-${this.formatNum(m)}-${this.formatNum(d)}`
            }else {
                this.choose = ``
            }
    ...
    .content {
        ...
        .days {
            ...
            .item {
                ...
                .today {
                    color: #fff;
                    background-color: rgba(246, 181, 77, 0.39);
                }
                .workDay {
                    font-style: normal;
                    width: 8rpx;
