<template>
  <view
    class="ask_detail"
    :class="userInfo.fake == '是' && detailInfo.status == 1 ? 'fake_btns' : ''"
  >
    <view
      class="top_tips"
      v-if="userInfo.fake == '是' && detailInfo.status == 1"
    >
      当前用户共发布{{ detailInfo.user_content_all }}篇专栏，已审核通过{{
        detailInfo.user_content_public_all
      }}篇，通过率为{{ detailInfo.user_content_public_percent }}%
    </view>
