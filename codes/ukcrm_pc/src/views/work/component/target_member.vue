    ...
    >
      ...
      >
        <div class="form d-flex">
          <div class="item">
            <el-form-item prop="member" label="设置员工">
              <el-autocomplete
...
export default {
  ...
  methods: {
    init(v) {
      ...
      this.$nextTick(() => {
        ...
        if (v.id) {
          ...
          this.form.member = `${v.name}(${v.phone})`;
          this.activeMember = {
            user_id: v.id,
            value: v.name,
          };
    ...
    entry() {
      getData({
        url: "/achievement/user_induction_time",
        data: {
        ...
        if (code === 200) {
          induction.forEach((a, b) => {
            let n = a.month.slice(a.month.length - 2, a.month.length); // 01到12
            this.form.month_list.forEach((v, k) => {
              if (n == this.format(v.num)) {
                v.status = a.status;
              }
            });
          });
    ...
    adver() {
      ...
      if (all > 0) {
        let a = Math.floor((all / len) * 100) / 100;
        let b = Math.round((all - len * a) * 100) / 100;
        this.form.month_list.forEach((v, k) => {
          if (v.status) {
            if (v.num == 12) {
              v.val = a + b;
    ...
    reset() {
      this.form.all = "";
      this.form.quart_list.forEach((v, k) => {
        v.val = 0;
      });
      this.form.month_list.forEach((v, k) => {
        v.id = 0;
