<template>
    <div class="add">
        <el-drawer :visible="true" direction="rtl" :before-close="cancel" :show-close="false" :withHeader="false"
            ...
            <div> 
                <el-form ref="project" :model="project" label-width="100px" :rules="rules" :inline="true" class="form-wrap"  label-position="right">
                    <el-form-item label="项目名称：" prop="example[0].name">
                        <el-input v-model="project.example[0].name" placeholder="请输入"></el-input>
                    </el-form-item>
                    <el-form-item label="项目周期：" prop="example[0].period" class="width3 flex-start" >
                    ...
                    <el-form-item label="演示网址：" prop="example[0].url">
                        <el-input v-model="project.example[0].url" placeholder="https://www.ebaina.com/"></el-input>
                    </el-form-item>
...
export default {
    ...
    data() {
        return {
            ...
            rules:{
                ...
                // ],
                // 匹配www,http开头的一切网址  /[a-zA-Z0-9][-a-zA-Z0-9]{0,62}(\.[a-zA-Z0-9][-a-zA-Z0-9]{0,62})+\.?/
                'example[0].url' : [
                    {pattern:/((http|ftp|https|file):\/\/([\w\-]+\.)+[\w\-]+(\/[\w\u4e00-\u9fa5\-\.\/?\@\%\!\&=\+\~\:\#\;\,]*)?)/ig,  message: '请输入正确的演示网址',  trigger: ['change','blur'] },
                ],
                'example[0].desc': [
    ...
    // console.log(regexp.test('https://www.baidu.com'));
        this.questionIndex()
