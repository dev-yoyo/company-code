<template>
    <div class="add">
        <el-drawer :visible="true" direction="rtl" :before-close="cancel" :show-close="false" :withHeader="false"
            ...
            <h4 class="title">编辑文件</h4>
            <div>
                <el-form ref="fileForm" :rules="rules" :model="fileForm" label-width="100px" class="form-wrap"
                    :inline="true" label-position="right">
                    <el-form-item label="文件名称：" prop="name">
                        <el-input placeholder="http://" class="el-select-w5" v-model="fileForm.name"></el-input>
                    </el-form-item>
                    ...
                    <el-form-item label="执行人" style="width:100%" prop="charge_user_id">
                        <el-select v-model="fileForm.charge_user_id" placeholder="请选择" class="el-select-w5">
                            <el-option v-for="(item, index) in selectOption" :key="index" :label="item.stage_name+(item.name!=null? '（' +item.name+'）':'')" v-if="projShow"
                                :value="item.id"></el-option>
                                <el-option v-for="(item, index) in selectOption" :key="index" :label="item.stage_name" v-if="!projShow"
                                :value="item.id"></el-option>
                                
                        </el-select>
                    </el-form-item>
                    <el-form-item label="交付时间:" prop="time">
                        <el-date-picker style="width:2rem" v-model="fileForm.time" type="date"
                            value-format="yyyy-MM-dd HH:mm:ss" placeholder="选择日期" :picker-options="pickerOptionsEnd">
                        </el-date-picker>
                    </el-form-item>
                    <el-form-item>
                        <div class="module-center">
                            <el-button type="" class="canclel" @click="cancel()">取消</el-button>
                            <el-button type="primary" class="confirm" @click="projectDocumentUpdate('fileForm')">确定</el-button>
                        </div>
                    </el-form-item>
                </el-form>
            </div>
...
export default {
    props: ['fileId', 'fileRow', 'currentUrl', 'queryId'],
    data() {
        return {
            fileForm: {
                project_document_id: '',
    ...
    created() {
        ...
        // this.fileForm = this.fileRow
        this.fileForm.project_document_id = this.fileRow.id
        this.fileForm.name = this.fileRow.name
        this.fileForm.demand = this.fileRow.demand
        this.fileForm.charge_user_id = this.fileRow.charge_user_id
        this.fileForm.time = new Date(this.fileRow.time)
    ...
    methods: {
        ...
        // 更新
        projectDocumentUpdate(formName) {
            var data = this.fileForm
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    postData({
                        url: "/workbech/" + this.currentUrl + "/project_document/update/",
                        data
                    }).then((res) => {
                        if (res.code == 200) {
