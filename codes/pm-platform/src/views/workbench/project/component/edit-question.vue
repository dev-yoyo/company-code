            ...
            <div> 
                <el-form ref="questionForm" :model="questionForm" label-width="100px"  :rules="rules" class="form-wrap" :inline="true" label-position="right">
                    ...
                    <el-form-item label="截止日期：" prop="end_time">
                        <el-date-picker placeholder="截止日期" value-format="yyyy-MM-dd HH:mm:ss" type="date" v-model="questionForm.end_time" 
                        :picker-options="pickerOptionsEnd" style="width:2rem" >
                        </el-date-picker>
                    </el-form-item>
                    ...
                    <el-form-item>
                        <div class="module-center">
                            ...
                            <el-button type="primary" class="confirm" @click="confirmQues('questionForm')">确定</el-button>
                        </div>
                    </el-form-item>
                </el-form>
...
export default {
    props:['queryId','currentUrl','quesRow'],
    data() {
        return {
            questionForm:{
                project_question_id:'', //	是	string	项目id
                desc	:'', //	是	string	问题描述
                pic	:[], //	否	string	问题图片
                charge_user_id:'',//
    ...
    this.questionForm.desc=this.quesRow.desc
    this.questionForm.pic=this.quesRow.pic
    this.questionForm.charge_user_id=this.quesRow.charge_user_id
    this.questionForm.copy_user_id=this.quesRow.copy_user_id
    this.questionForm.type=this.quesRow.type
    this.questionForm.degree_importance=this.quesRow.degree_importance
    this.questionForm.priority_level=this.quesRow.priority_level
    this.questionForm.end_time=this.quesRow.end_time  
    console.log(this.questionForm.end_time);
    },
    methods: {
        cancel() {
            this.$emit("cancel", {});
        },
        confirmQues(formName){
            var data=this.questionForm
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    postData({
            url: "workbech/"+this.currentUrl+"/project_question/update",
            data
        }).then((res) => {
            if (res.code==200) {
