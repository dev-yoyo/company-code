          ...
          <div class="felx-box basic-infor">
            <el-form label-width="100px">
              <div class="basicline jbxx">
                ...
                <span class="flex-center mt2" v-if="!inforStatus">
                  <el-form-item label="姓名：">
                    <div class="flex-box tdmc">
                      ...
                      <svg class="icon" aria-hidden="true" v-if="dataList.status == '未提交'">
                        <use xlink:href="#icon-qj-wrz"></use>
                      </svg>
                      <svg class="icon" aria-hidden="true" v-if="dataList.status == '已认证'">
                  ...
                  <el-form-item label="工作年限：">
                    <span v-if="dataList.work_year"> {{ dataList.work_year }}年</span>
                  ...
                  <el-form-item label="可胜任岗位：" style="width:100%">
                    <div>
                      <span v-for="(item, index) in dataList.jobs" :key="index">{{ item }} &nbsp;</span>
                    </div>
                    <!-- <el-button v-for="(item,index) in dataList.jobs" :key="index">{{item}}</el-button> -->
                  </el-form-item>
                </span>
                <div class="edit_status mt2" v-if="inforStatus">
                  ...
                  <el-form-item label="工作年限：">
                    <input type="number" v-model="dataList.work_year">
                  </el-form-item>
                  <el-form-item label="所在地区：">
                    <!-- <input type="text" v-model="dataList.area"> -->
                    <!-- <input type="text" v-model="dataList.area"> -->
                    <el-cascader v-model="checkArea" :options="areaList" @change="chooseArea"></el-cascader>
                  </el-form-item>
                  <el-form-item label="公司名称：">
                    <input type="text" v-model="dataList.company">
                  </el-form-item>
                  <el-form-item label="职位名称：">
                    <input type="text" v-model="dataList.position">
                  </el-form-item>
                  <el-form-item label="所属行业：">
                    <input type="text" v-model="dataList.industry">
                  </el-form-item>
                  <el-form-item label="可胜任岗位：" class="srgw">
                    <el-select v-model="dataList.jobs" multiple :multiple-limit="5">
                ...
                <div v-if="!teamStatus">
                  <el-form-item label="擅长平台：">
                    <span v-if="dataList.handle_platform">
                      <el-button class="tag-btn" v-for="(item, index) in dataList.handle_platform" :key="index">{{
                        item
                      }}</el-button>
                    </span>
                  </el-form-item>
                  <el-form-item label="技能掌握：">
                    <span v-if="dataList.skill">
                      <el-button class="tag-btn" v-for="(item, index) in dataList.skill" :key="index">{{
                        item
                      }}</el-button>
                    </span>
                  </el-form-item>
            ...
            <el-form>
              <div class="basicline tdjs">
                <div class="flex-center xx-title gznl">
                  <h3>工作能力</h3>
                  <el-button v-if="!abilityStatus" class="edit-user" @click="editinfor('3')">编辑</el-button>
                  <div v-if="abilityStatus">
                    <el-button class="edit-user" @click="abilityStatus = false; seniorityIndex()">取消</el-button>
                    <el-button class="edit-user" @click="seniorityUpdate('1')">保存</el-button>
                  </div>
                </div>
                <el-form-item label="" v-if="!abilityStatus" class="item-ability">
                  {{ dataList.ability }}
                </el-form-item>
                <div class="edit_status" v-if="abilityStatus">
                  <el-form-item>
                    <textarea name="" id="" cols="30" rows="10" v-model="dataList.ability" maxlength="300"></textarea>
                  </el-form-item>
                </div>
              </div>
            </el-form>
        ...
        <section class="cyxm">
          <div class="participate-project">
            <div class="participate-title flex-center">
              <h3> 个人简历<span class="color_666 font12">（上传工作简历，在项目竞价中更有优势哦~） </span></h3>
              <div class="flex-center">
                ...
                <div class="flex-center" v-if="dataList.path_url">
                  ...
                  <span class="docu-name com-line-1">个人简历</span>
                </div>

                <el-upload class="upload-demo" drag multiple action="" :show-file-list="false"
            ...
            <div class="qyxm-infor">
              ...
              <div class="align-center no-data" v-if="dataList.example == null || dataList.example.length == 0">
                <img src="https://ebaina.oss-cn-hangzhou.aliyuncs.com/dev/direct/resource/202211/22/gzt/nodata.png"
                  alt="">
                <p>暂无数据</p>
              </div>
                ...
                <div class="lcb-list-item">
                  <div shadow="hover" class="lcb-ts-item">
                    <div class="flex-center ts-head">
                      ...
                      <!-- <el-button type="primary" class="list-btn-add" @click="hideXmal=!hideXmal">
                        {{hideXmal?'收起':'展开'}}
                      </el-button> -->
                      <span class="list-btn-add active"
                        @click="packUp(`table${index}`, 'block', `open${index}`, `close${index}`)"
                        :ref="'close' + index">展开

                      </span>
                      <span class="list-btn-add"
                        @click="packUp(`table${index}`, 'none', `open${index}`, `close${index}`)"
                        :ref="'open' + index">收起
                    ...
                    <div class="alxx">
                      ...
                      <div class="hide-xmal" :ref="'table' + index">
                        ...
                        <div v-for="(v, i) in item.pic" :key="i">
                          <img :src="v" alt="" class="pro_img">
                        </div>
                        <!-- <img src="https://ebaina.oss-cn-hangzhou.aliyuncs.com/dev/direct/resource/202211/22/home/banner.png" alt="">
...
export default {
  ...
  methods: {
    chooseArea(v) {
      this.dataList.area=this.checkArea.toString().replaceAll(',',' ')
    },
    // 全国省市区
    getArea() {
      getData({ url: "/common/get_area" }).then((res) => {
        const {
          code,
          data: { list },
        } = res;
    ...
    // 个人资历修改
    seniorityUpdate(type) {
      var data = {
        work_year: this.dataList.work_year,
        area: this.dataList.area,
        company: this.dataList.company,
        position: this.dataList.position,
        industry: this.dataList.industry,
        jobs: this.dataList.jobs,
        skill: this.dataList.skill,
        handle_platform: this.dataList.handle_platform,
        ability: this.dataList.ability,
    ...
    editinfor(type) {
      ...
      else if (type == 3) this.abilityStatus = true
    },
    // 列表查询
    seniorityIndex() {
